import { PRESET_LOCATIONS, INITIAL_INCIDENTS, INITIAL_RESOURCES, INITIAL_SHELTERS, INITIAL_ROADS, INITIAL_SOS_REPORTS, INITIAL_DAMAGE_REPORTS } from './presetsData.js';
...
export class Store {
    ...
    setCustomBounds(bounds) {
        ...
        if (!this.incidents.has('custom'))
            this.incidents.set('custom', []);
        if (!this.resources.has('custom'))
            this.resources.set('custom', []);
        if (!this.shelters.has('custom'))
            this.shelters.set('custom', []);
        if (!this.roads.has('custom'))
            this.roads.set('custom', []);
        if (!this.sosReports.has('custom'))
            this.sosReports.set('custom', []);
        if (!this.damageReports.has('custom'))
            this.damageReports.set('custom', []);
        return customPreset;
    }
    getIncidents(presetId = this.currentPresetId) {
    ...
    addIncident(incident, presetId = this.currentPresetId) {
    ...
    updateIncident(incident, presetId = this.currentPresetId) {
        const list = this.getIncidents(presetId);
    ...
    getSOSReports(presetId = this.currentPresetId) {
        return this.sosReports.get(presetId) || [];
    }
    getSOSByIncidentId(incidentId) {
    ...
    addSOSReport(report, presetId = this.currentPresetId) {
    ...
    updateSOSReport(id, updates) {
        for (const [pid, list] of this.sosReports.entries()) {
            const idx = list.findIndex(s => s.sos_id === id || s.id === id);
            if (idx >= 0) {
                list[idx] = { ...list[idx], ...updates };
                if (updates.urgency)
                    list[idx].severity = updates.urgency;
                if (updates.severity)
                    list[idx].urgency = updates.severity;
                if (updates.people_count !== undefined)
                    list[idx].people_affected = updates.people_count;
    ...
    getDamageReports(presetId = this.currentPresetId) {
        return this.damageReports.get(presetId) || [];
    }
    addDamageReport(report, presetId = this.currentPresetId) {
    ...
    getResources(presetId = this.currentPresetId) {
        ...
        for (const [pid, list] of this.resources.entries()) {
            ...
            if (idx >= 0) {
                ...
                if (updates.availability)
                    list[idx].status = updates.availability;
                if (updates.status)
                    list[idx].availability = (updates.status === 'AVAILABLE' ? 'AVAILABLE' : updates.status === 'ASSIGNED' ? 'ASSIGNED' : updates.status === 'RESERVED' ? 'RESERVED' : 'UNAVAILABLE');
    ...
    assignResource(resourceId, incidentId, presetId = this.currentPresetId) {
    ...
    setSARResult(result, presetId = this.currentPresetId) {
        this.sarResults.set(presetId, result);
        this.runFusion(presetId);
    }
    runFusion(presetId = this.currentPresetId) {
