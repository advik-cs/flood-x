import { DamageReport, Incident, ResourceItem, RoadEdge, ShelterItem, SOSReport, DroneDetection, SARAnalysisResult } from '../types.js';
...
export class FusionEngine {
  public fuse(input: FusionInput, existingIncidents: Incident[]): Incident[] {
    ...
    const updatedIncidents = existingIncidents.map(inc => {
      ...
      for (const r of roads) {
        for (const [rLat, rLng] of r.coordinates) {
          if (routingService.calculateDistanceKm({ lat: rLat, lng: rLng }, inc.location) < 1.0) {
            if (r.status === 'BLOCKED') roadStatus = 'BLOCKED';
            else if (r.status === 'PARTIALLY_BLOCKED' && roadStatus !== 'BLOCKED') roadStatus = 'PARTIALLY_BLOCKED';
          }
        }
      }
      ...
      if (linkedDamage.some(d => d.damage_level === 'CRITICAL')) damageLevel = 'CRITICAL';
      else if (linkedDamage.some(d => d.damage_level === 'SEVERE')) damageLevel = 'SEVERE';

      // 5. Correlate nearest shelter
      let nearestShelter: ShelterItem | null = null;
