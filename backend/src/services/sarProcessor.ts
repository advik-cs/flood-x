import { BoundingBox, SARAnalysisResult } from '../types.js';

export interface SARProcessOptions {
  aoi_name: string;
  bounds: BoundingBox;
  threshold_db?: number;
  mode?: 'SENSITIVE' | 'STANDARD' | 'STRICT' | 'CUSTOM';
  water_cutoff_db?: number;
...
export interface PixelProbeResult {
  ...
  post_db?: number;
  delta_db?: number;
  classification: 'INUNDATED (FLOOD EXTENT)' | 'CRITICAL BREACH CORE' | 'PERMANENT WATER BODY' | 'NORMAL / UNFLOODED TERRAIN' | 'OUTSIDE AOI';
...
export interface RasterCacheEntry {
  ...
  deltaRaster: number[][];
  finalMask: boolean[][];
  highSeverityMask: boolean[][];
  permanentWaterMask: boolean[][];
  thresholdDb: number;
  waterCutoff: number;
  isBengaluru: boolean;
}
...
export class SARProcessor {
  private rasterCache: Map<string, RasterCacheEntry> = new Map();
  private activeRaster: RasterCacheEntry | null = null;
  ...
  public process(options: SARProcessOptions): SARAnalysisResult {
    const {
      aoi_name,
      bounds,
    ...
    if (mode === 'SENSITIVE') thresholdDb = -2.0;
    else if (mode === 'STANDARD') thresholdDb = -3.0;
    else if (mode === 'STRICT') thresholdDb = -4.5;
    else mode = 'CUSTOM';
    ...
    const preRaster: number[][] = [];
    const postRaster: number[][] = [];
    const deltaRaster: number[][] = [];
    ...
    if (isBengaluru) {
      ...
      const distToRajakaluve = (r: number, c: number) => {
        if (c < 20 || c > 92) return 999;
        const t = (c - 20) / 72;
        const channelR = 43 - 8 * Math.sin(t * Math.PI) + 4 * Math.sin(t * 3 * Math.PI);
        return Math.abs(r - channelR);
      };

      const bellandurLakeDist = (r: number, c: number) => {
        const dr = (r - 43) / 6.5;
        const dc = (c - 24) / 7.5;
        return Math.sqrt(dr * dr + dc * dc);
      };

      const varthurLakeDist = (r: number, c: number) => {
        const dr = (r - 33) / 7.0;
        const dc = (c - 86) / 8.0;
        return Math.sqrt(dr * dr + dc * dc);
      };
      ...
      const tributaryCol = (r: number) => {
        const t = r / ROWS;
        if (t < 0.30) return -999;
      ...
      const terrainElevation = (r: number, c: number, seedVal: number) => {
        const nr = r / ROWS;
        const nc = c / COLS;
    ...
    let scaleFactor = 1.0;
    if (mode === 'SENSITIVE') scaleFactor = 1.25;
    else if (mode === 'STRICT') scaleFactor = 0.75;
    else if (thresholdDb > -2.5) scaleFactor = 1.15;
    else if (thresholdDb < -4.0) scaleFactor = 0.80;
    ...
    const permanentWaterMask: boolean[][] = [];
    const highSeverityMask: boolean[][] = [];
    ...
    const cacheEntry: RasterCacheEntry = {
      aoi_name,
      bounds,
      ROWS,
      COLS,
  ...
  public probePoint(lat: number, lng: number, aoi_name?: string): PixelProbeResult {
    let matchedRaster: RasterCacheEntry | null = null;
    ...
    let classification: PixelProbeResult['classification'];
    let severity: PixelProbeResult['severity'];
  ...
  private rasterMaskToPolygons(
    ...
    const resultPolygons: [number, number][][][] = [];

    const roundCoord = (v: number) => Math.round(v * 100000) / 100000;
    const coordKey = (p: [number, number]) => `${roundCoord(p[0])},${roundCoord(p[1])}`;
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        const edgeMap = new Map<string, { from: [number, number]; to: [number, number]; used: boolean }[]>();

        const addDirectedEdge = (from: [number, number], to: [number, number]) => {
        ...
        for (const [, edges] of edgeMap.entries()) {
          for (const edge of edges) {
            ...
            const ring: [number, number][] = [edge.from];
        ...
        const smoothedRings = rings.map(rawRing => {
          ...
          const simplified: [number, number][] = [rawRing[0]];
          ...
          if (simplified.length < 4) return rawRing;
