...
import { DroneDetection, GeoPoint } from '../types.js';
...
export class VisionService {
  ...
  private async getOrLoadModel(): Promise<cocoSsd.ObjectDetection | null> {
  ...
  private decodeImageFile(filePath: string): { width: number; height: number; rgbData: Int32Array } {
  ...
  private applyNMS(detections: DroneDetection[], iouThreshold = 0.35): DroneDetection[] {
  ...
  public async detectInImageFile(
    filePath: string,
    filename: string,
    imageId: string,
    georefOptions?: GeoreferenceOptions
    ...
    const candidateDetections: DroneDetection[] = filteredCandidates.map((c) => {
      ...
      if (
        georefOptions?.latitude !== undefined &&
  ...
  public analyzeImage(
    ...
    clientDetections?: DroneDetection[],
    georefOptions?: GeoreferenceOptions,
    clientInferenceTime?: number
    ...
    if (isDemoFallback) {
      ...
      if (clientDetections && Array.isArray(clientDetections)) {
        detections = clientDetections.map((d, index) => {
          ...
          const updatedDet: DroneDetection = {
            id: detId,
            image_id: imageId,
